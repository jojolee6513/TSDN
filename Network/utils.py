...
def sample_wise_normalization(X, axis=0):
    if axis == 1:
...
def categorical_crossentropy(preds, labels):
    return np.mean(-np.log(np.extract(labels, np.clip(preds, 1e-8, 1))))
...
def evaluate_preds(preds, labels, indices, epoch, NB_EPOCH, weight_dis):
    ...
    for y_split, idx_split in zip(labels, indices):
        split_loss.append(sparse_crossentropy(preds[idx_split], y_split[idx_split]) -weight_dis*np.mean(np.sum(np.multiply(softmax(preds), np.log(softmax(preds))),axis=1)))
...
def softmax_loss(logits, labels):
    cross_entropy = K.sparse_categorical_crossentropy(target=labels, output=(K.softmax(logits)))
